...
from hyper_nerf_gan.src.nerf.run_nerf_helpers import *

from hyper_nerf_gan.src.nerf.load_llff import load_llff_data
from hyper_nerf_gan.src.nerf.load_deepvoxels import load_dv_data
from hyper_nerf_gan.src.nerf.load_blender import load_blender_data
from hyper_nerf_gan.src.nerf.load_LINEMOD import load_LINEMOD_data
...
def render(H,
    ...
    if c2w is not None:
        ...
        if patch_size is not None:
            ...
            if index_channel:
                index_channels = make_indexing_channels(H, channel_first=False, low=0)[crop_w:crop_w + patch_size,
                                 crop_h:crop_h + patch_size]
    else:
...
def raw2outputs(raw, z_vals, rays_d, raw_noise_std=0, white_bkgd=False, pytest=False):
    ...
    dists = dists.to(device=raw.device)
    ...
    depth_map = torch.sum(weights * z_vals.to(device=weights.device), -1)
